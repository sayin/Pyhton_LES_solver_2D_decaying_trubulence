...
#%%
def compute_stress_horiuti(nx,ny,nxc,nyc,dxc,dyc,u,v,n,ist,ics,ifltr,alpha):
    ...
    CH2 = compute_cs_horiuti(dxc,dyc,nxc,nyc,uc,vc,a11,a12,a22,ics,ifltr,alpha) # for Smagorinsky
    
    print(n, " CH = ", (np.abs(np.max(CH2)))**(1/2), " ", (np.abs(np.min(CH2)))**(1/2),
          " ", np.mean((CH2))**(1/2), " ", np.std((CH2))**(1/2))
...
def compute_stress(nx,ny,nxc,nyc,dxc,dyc,u,v,n,ist,ics,ifltr,alpha):
    if ist == 1 or ist == 5:
        ...
        compute_stress_horiuti(nx,ny,nxc,nyc,dxc,dyc,u,v,n,ist,ics,ifltr,alpha)
    elif ist == 4:
...
for n in range(ns-1,ns+1):
    ...
    u = sy
    v = -sx
    compute_stress(nx,ny,nxc,nyc,dxc,dyc,u,v,n,ist,ics,ifltr,alpha)
